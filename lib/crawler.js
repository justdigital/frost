  ...
  page.open(url, function (status) {
    if ('fail' === status) {
      ...
      var watchEnded = function(err) {
        // If there is an error, shouts for the failure
        // callback
        if (err) {
          self.onFailure({
            url: url,
          ...
          // If this is the crawling mode, adds the page's links to
          // the queue
          if (self.mode === "crawl") {
          ...
          // Sucessfully loaded page, calls success callback to save
          // the file
